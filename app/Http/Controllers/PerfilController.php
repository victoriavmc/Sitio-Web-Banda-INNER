...
class PerfilController extends Controller
{
    ...
    public function reportado(Request $request)
    {
        ...
        switch ($decision) {
            case 0:
                ...
                break;
            case 1:
                #En caso que el reporte sea necesario pero no es muy problematico, entro a la tabla historialUsuario y desactivamos por tiempo definido
                // El idReportado debe coincidir con el idUsuario que presenta dentro de la tabla idDatosPersonales
                $DatosPersonales = DatosPersonales::where('idusuarios', $idReportado)->first();
                $idDP = $DatosPersonales->idDatosPersonales;
                // Busco la relación entre el usuario que se quiere desactivar y el usuario actual
                $baneado = HistorialUsuario::where('datospersonales_idDatosPersonales', $idDP)->first();
                if ($baneado) {
                    $baneado->estado = 'Inactivo';
                    $baneado->fechaInica = $request->fechaInicia;
                    $baneado->fechaFinal = $request->fechaFinal;
                    $baneado->save();
                    break;
                }

                // ENVIAR UN MAIL DONDE SE LE INFORME QUE VA A ESTAR BANEADO DE LA PAGINA POR X CANTIDAD DE DIAS
            ...
            case 2:
                ...
                if ($baneado) {
                    $baneado->estado = 'Inactivo';
                    $baneado->eliminacionLogica = 'Si';
                    $baneado->save();

                    //ELIMINAR EN CADENA TODOS LOS DATOS DEL USUARIO.
                    break;
                }
                // ENVIAR UN MAIL DONDE SE LE INFORME QUE VA A ESTAR ELIMINADO DE LA PAGINA, NO PUEDE VOLVER A INGRESAR
        }
    }
