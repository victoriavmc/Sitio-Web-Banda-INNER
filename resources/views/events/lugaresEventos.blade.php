        ...
        <div class="rounded-lg max-w-max mx-auto shadow-xl bg-white">
            <nav class="flex min-w-[240px] flex-col gap-1 p-1.5">
                @foreach ($lugares as $lugar)
                    <div
                        class="text-black flex justify-between gap-5 w-full items-center rounded-md p-2 pl-3 transition-all hover:bg-slate-600 focus:bg-slate-600 active:bg-slate-600">
                        <p>{{ $lugar->nombreLugar . ', ' . $lugar->localidad . ', ' . $lugar->calle . ' ' . $lugar->numero }}
                        </p>
                        <div class="grid grid-cols-2 place-items-center">
                            <button
                                class="rounded-md border border-transparent p-2.5 text-center text-sm transition-all text-black hover:bg-slate-700 focus:bg-slate-700 active:bg-slate-700 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
