<x-AppLayout>
    <div class="bg-white min-h-[86vh] p-10 bg-cover">
                ...
                <div class="flex flex-wrap justify-between gap-5">
                    @foreach ($listaYt as $index => $video)
                        <div class="video-container mb-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/5"> <!-- Ancho adaptativo -->
                            <iframe src="{{ $video['linkYt'] ?? '#' }}" width="100%" height="200" frameborder="0"
                                allowfullscreen></iframe>
                            <h1 class="text-lg font-bold mt-2">{{ $video['titulo'] }}</h1> <!-- Título destacado -->
                            <p class="text-sm text-gray-600">{{ $video['fecha'] }}</p> <!-- Fecha en un tono gris -->
                        </div>
